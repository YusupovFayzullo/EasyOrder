...
public record UserCreateDTO(@Size(
...
) String email, boolean isOAuthUser) implements Serializable {
    public UserCreateDTO(@Size(
            ...
            regexp = "^[\\w-\\.]+@([\\w-]+\\.)+[\\w-]{2,4}$",
            message = "Email is not valid"
    ) String email, @Parameter(description = "If user is OAuth user", example = "false") boolean isOAuthUser) {
    ...
    @Parameter(
            description = "If user is OAuth user",
            example = "false"
    )
